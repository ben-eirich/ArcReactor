...
namespace ArcReactor {
    public sealed class HashChain8 : IMatchFinder {
        int WindowBits;
        int WindowSize;
        int WindowMask;
        ...
        public HashChain8(byte[] data, int windowBits = 21, int maxProbes = 16) {
            Initialize(data, windowBits, maxProbes, windowBits);
        }

        public HashChain8(byte[] data, int windowBits, int maxProbes, int hashBits) {
        ...
        int Hash(int idx) {
            return Hash4(idx) ^ Hash4(idx + 4);
        }

        int Hash4(int idx) {
            if (idx > InputSize - 8) return 0;
            int val = Data[idx + 0] | (Data[idx + 1] << 8) | (Data[idx + 2] << 16) | (Data[idx + 3] << 24);
            int x = val;
            x ^= x >> 18;
